...
pub async fn register_ws_client(
            ...
            match client {
                Some(_) => {
                    return Err(AppError::new(
                        "websocket client already registed",
    ...
    match req.jwt_token {
        Some(token) => {
            ...
            match payload {
                ...
                Ok(payload) => {
                    user_id = Some(payload.claims.user_id);
                }
            }
        }
        None => {}
    ...
    let ws_url = format!(
        "ws://{}:{}/{}?uuid={}",
        backend_host, backend_port, ws_endpoint, uuid
    );
...
pub async fn make_ws_connection(
    ws: warp::ws::Ws,
    ...
    match client {
        Some(c) => Ok(ws.on_upgrade(move |socket| {
            client_connection(socket, query_params.uuid, client_list, c)
        })),
        None => {
...
async fn client_connection(
    ...
    while let Some(result) = client_ws_rcv.next().await {
        ...
        let _ = match result {
            ...
            Err(e) => {
                eprintln!(
                    "error receiving ws message for uuid: {}): {}",
                    uuid.clone(),
                    e
                );
                break;
