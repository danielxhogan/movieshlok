...
async fn movie_details(
    ...
    let response = reqwest::get(&movie_details_url)
        .await
        .unwrap()
        .json::<MovieDetails>()
        .await
...
async fn person_details(
    ...
    let person_details_url = format!(
        "{}/person/{}?api_key={}&append_to_response=credits&language=en-US",
        &tmdb_base_url, &person_details_params.person_id, &tmdb_api_key
    );

    let response = reqwest::get(&person_details_url)
        .await
        .unwrap()
        .json::<PersonDetails>()
        .await
